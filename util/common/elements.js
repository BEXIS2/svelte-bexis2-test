...
async function filterTable_Telerik(page, filterText) {
  ...
  await page.waitForSelector('#divResultGrid > div > table > thead > tr > th:nth-child(1) > div', {
    visible: true
  });

  await page.click('#divResultGrid > div > table > thead > tr > th:nth-child(1) > div');

  await page.waitForSelector('.t-clear-button', {
    visible: true
  });
  ...
  await page.waitForSelector('.t-status-text', {
    visible: true
  });
...
async function filterTable_Telerik2(page, filterText, pos, id, id_text) {
  ...
  await assert.isFulfilled(page.waitForSelector('#' + id + ' .t-clear-button', {
    visible: true
  }), 'wait for open filter');

  await assert.isFulfilled(page.click(id_text, {
    clickCount: 3
  }), 'delete previous filter text');
  ...
  await assert.isFulfilled(page.waitForSelector('.t-status-text', {
    visible: true
...
async function sortTable(page, rowNumber) {
  ...
  await assert.isFulfilled(page.waitForSelector('.show_table', {
    hidden: true
  }), 'should wait for result table');

}

/**
 * Returns a column content between two selected rows of a table
 *
 * @param {Object} page
 * @param {string} table_id
 * @param {number} table_id must be an integer greater than zero
 * @param {number} col must be an integer greater than zero
 * @param {number} firstRow must be an integer greater than zero
 * @param {number} lastRow must be an integer greater than zero
 */


async function returnColumnContent(page, table_id, col, firstRow, lastRow) {

  return await page.evaluate((table_id, col, firstRow, lastRow) => {
    let tab = document.querySelector(table_id),
        n = tab.rows.length,
        arr = [],
        row;
    ...
    for (row = firstRow; row < n - lastRow; ++row) {
      if (tab.rows[row].cells.length > col) {
        arr.push(tab.rows[row].cells[col].textContent.trim());
      }
    }
    return arr;
  }, table_id, col, firstRow, lastRow);
}